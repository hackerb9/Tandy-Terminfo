# Comparing Tandy Terminfo to Heathkit H19 

The Heathkit h19 terminfo is so similar to the one used by the Tandy
200 that it seems likely that when the Kyotronic-85 was first
developed, output was probably to an h19 before the screen hardware
was working.

The h19 was a VT52 clone with additional features that make it more
similar to a Tandy portable

* Standout Mode (reverse video) is enabled with
  <kbd>Esc</kbd><kbd>p</kbd>, disabled with
  <kbd>Esc</kbd><kbd>q</kbd>.

* Clear screen is <kbd>Esc</kbd><kbd>E</kbd>.


## Differences

```
infocmp -L -d td200 h19 | tr , : | column -c102 -t -W2 -E2 -s: -N attribute,td200,h19 -o'|'  
```

attribute              |td200                             |h19
-|-|-
    comparing booleans.|                                  |
	auto_left_margin       | T                                |F.
	backspaces_with_bs     | F                                |T.
	dest_tabs_magic_smso   | T                                |F.
	has_status_line        | F                                |T.
	move_insert_mode       | F                                |T.
	move_standout_mode     | F                                |T.
	status_line_esc_ok     | F                                |T.
	xon_xoff               | T                                |F.
    comparing numbers. |                                  |
	columns                | 40                               | 80.
	lines                  | 16                               | 24.
    comparing strings. |                                  |
	acs_chars              | '+\232\054\233-\230.\2310\357`\23| '+h.kaiggjdkclfmenbozqas{tvutvuwsx`~\136'.
                       |5a\377f\246g\215h\345i\251j\367k\3|
                       |62l\360m\366n\372q\361t\364u\371v\|
                       |370w\363x\365{\210|\212}\243~\325'|
	cursor_invisible       | '\EQ'                            | NULL.
	cursor_left            | '\ED'                            | '^H'.
	cursor_normal          | '\EP'                            | '\Ey4'.
	cursor_visible         | NULL                             | '\Ex4'.
	delete_character       | NULL                             | '\EN'.
	delete_line            | '\EM'                            | NULL.
	dis_status_line        | '\EU\EY0 \ES\EM'                 | NULL.
	enter_alt_charset_mode | NULL                             | '\EF'.
	enter_insert_mode      | NULL                             | '\E@'.
	enter_reverse_mode     | '\Ep'                            | NULL.
	exit_alt_charset_mode  | NULL                             | '\EG'.
	exit_attribute_mode    | '\Eq'                            | NULL.
	exit_insert_mode       | NULL                             | '\EO'.
	from_status_line       | NULL                             | '\Ek\Ey5'.
	init_1string           | '\EU'                            | NULL.
	init_2string           | '\EW\Eq\EE'                      | NULL.
	insert_line            | '\EL'                            | NULL.
	insert_padding         | NULL                             | '$<1.5/>'.
	key_down               | '^_'                             | '\EB'.
	key_f1                 | NULL                             | '\ES'.
	key_f2                 | NULL                             | '\ET'.
	key_f3                 | NULL                             | '\EU'.
	key_f4                 | NULL                             | '\EV'.
	key_f5                 | NULL                             | '\EW'.
	key_f6                 | NULL                             | '\EP'.
	key_f7                 | NULL                             | '\EQ'.
	key_f8                 | NULL                             | '\ER'.
	key_home               | NULL                             | '\EH'.
	key_left               | '^]'                             | '\ED'.
	key_right              | '^\'                             | '\EC'.
	key_up                 | '^^'                             | '\EA'.
	lab_f6                 | NULL                             | 'blue'.
	lab_f7                 | NULL                             | 'red'.
	lab_f8                 | NULL                             | 'white'.
	newline                | '\r\n'                           | NULL.
	scroll_reverse         | NULL                             | '\EI'.
	to_status_line         | NULL                             | '\Ej\Ex5\EY8%p1%' '%+%c\Eo\Eo'.


## Common features

```
infocmp -L -c td200 h19 | column -c102 -t -s= -W2 -E2 -N attribute,both -o'|'
```

attribute               |both td200 and h19
-|-
    comparing booleans. |
	auto_right_margin       | T.
	back_color_erase        | F.
	can_change              | F.
	ceol_standout_glitch    | F.
	col_addr_glitch         | F.
	cpi_changes_res         | F.
	cr_cancels_micro_mode   | F.
	eat_newline_glitch      | F.
	erase_overstrike        | F.
	generic_type            | F.
	hard_copy               | F.
	hard_cursor             | F.
	has_meta_key            | F.
	has_print_wheel         | F.
	hue_lightness_saturation| F.
	insert_null_glitch      | F.
	lpi_changes_res         | F.
	memory_above            | F.
	memory_below            | F.
	needs_xon_xoff          | F.
	no_esc_ctlc             | F.
	no_pad_char             | F.
	non_dest_scroll_region  | F.
	non_rev_rmcup           | F.
	over_strike             | F.
	prtr_silent             | F.
	row_addr_glitch         | F.
	semi_auto_right_margin  | F.
	tilde_glitch            | F.
	transparent_underline   | F.
    comparing numbers.  |
	init_tabs               | 8.
    comparing strings.  |
	bell                    | '^G'.
	carriage_return         | '\r'.
	clear_screen            | '\EE'.
	clr_eol                 | '\EK'.
	clr_eos                 | '\EJ'.
	cursor_address          | '\EY%p1%' '%+%c%p2%' '%+%c'.
	cursor_down             | '\EB'.
	cursor_home             | '\EH'.
	cursor_right            | '\EC'.
	cursor_up               | '\EA'.
	enter_standout_mode     | '\Ep'.
	exit_standout_mode      | '\Eq'.
	key_backspace           | '^H'.
	scroll_forward          | '\n'.
	tab                     | '^I'.

## Notes

### Differences which are not actual

* cursor_left on the h19 is defined as ^H. The VT52 and Tandy Terminfo
  specify '\ED'. But both of those codes are synonyms and they work
  the same on all three terminals.

* Tandy Terminfo's enter_reverse_mode is just an alias for
  enter_standout_mode. Likewise exit_attribute_mode for
  exit_standout_mode.

* newline is not defined in the h19 terminfo, but probably should be
  the same ('\r\n').

### Differences which may be bugs in my Tandy terminfo

* move_standout_mode: I tested and it is safe to move in standout mode, so this ought to be True. 

### Differences which look like bugs but aren't

* has_status_line: What the Tandy portables call a "status line" is
  very different from a typical UNIX status line such as on the h19.

* auto_left_margin: The Tandy 200 definitely goes up to the previous
  line when backspacing from column 0. 

* dest_tabs_magic_smso: This means "destructive tabs, magic so char"
  While primarily known as a glitch in the Teleray t1061 terminal,
  this problem also affects the Tandy computers. Specifically a Tab
  character will clear any text that it passes over. 

* backspaces_with_bs (OTbs): I cannot find any description of this
  attribute other than that it is an "obsolete termcap capability". 
  It actually is set in the Tandy terminfo, but modern versions of tic
  simply strip it out.

### Non-terminfo additions:

* u9: terminal enquire string
* u8: terminal answerback description


